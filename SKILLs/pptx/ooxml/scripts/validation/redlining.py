...
class RedliningValidator:
    ...
    def validate(self):
        ...
        try:
            ...
            # 只有在使用了 Claude 的修订跟踪时才需要进行修订验证
            if not claude_del_elements and not claude_ins_elements:
                if self.verbose:
                    print("通过 - 未找到 Claude 的修订跟踪。")
                return True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if self.verbose:
                print("通过 - Claude 的所有修改都已正确跟踪")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """使用 git 词汇差异生成详细的词汇级差异。"""
        error_parts = [
            "失败 - 移除 Claude 的修订跟踪后文档文本不匹配",
            "",
            "可能的原因：",
        ...
        if git_diff:
            error_parts.extend(["差异详情：", "============", git_diff])
        else:
            error_parts.append("无法生成词汇差异（git 不可用）")

        return "\n".join(error_parts)

    def _get_git_word_diff(self, original_text, modified_text):
        """使用 git 生成具有字符级精度的词汇差异。"""
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                temp_path = Path(temp_dir)
            ...
            # Git 不可用或其他错误，返回 None 以使用备用方案
            pass

        return None

    def _remove_claude_tracked_changes(self, root):
        """从 XML 根节点中移除 Claude 创作的修订跟踪。"""
    ...
    def _extract_text_content(self, root):
        ...
        跳过空段落以避免当修订插入仅添加结构元素而无文本内容时产生误报。
        """
