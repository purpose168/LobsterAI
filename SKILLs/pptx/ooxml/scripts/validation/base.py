...
class BaseSchemaValidator:
    ...
    # scope 可以是 'file'（文件内唯一）或 'global'（全局唯一）
    ...
    def validate_unique_ids(self):
        """验证特定 ID 是否符合 OOXML 要求的唯一性。"""
        errors = []
        global_ids = {}  # 跟踪所有文件中全局唯一的 ID

        for xml_file in self.xml_files:
            try:
                root = lxml.etree.parse(str(xml_file)).getroot()
                file_ids = {}  # 跟踪此文件内必须唯一的 ID
    ...
    def validate_all_relationship_ids(self):
        """
        验证 XML 文件中的所有 r:id 属性是否引用其对应 .rels 文件中的现有 ID，
        并可选地验证关系类型。
        """
        import lxml.etree
    ...
    def validate_content_types(self):
            ...
            for xml_file in self.xml_files:
                ...
                try:
                    ...
                    if root_name in declarable_roots and path_str not in declared_parts:
                        errors.append(
                            f"  {path_str}: 具有 <{root_name}> 根的文件未在 [Content_Types].xml 中声明"
                        )
        ...
        if errors:
            ...
            if self.verbose:
                print(
                    "通过 - 所有内容文件在 [Content_Types].xml 中正确声明"
                )
            return True

    def validate_file_against_xsd(self, xml_file, verbose=False):
        """根据 XSD 架构验证单个 XML 文件，并与原始文件比较。
        ...
        返回:
            元组: (is_valid, new_errors_set) 其中 is_valid 为 True/False/None（已跳过）
        """
        # 解析两个路径以处理符号链接
    ...
    def _validate_single_file_xsd(self, xml_file, base_path):
        """根据 XSD 架构验证单个 XML 文件。返回 (is_valid, errors_set)。"""
    ...
    def _remove_template_tags_from_text_nodes(self, xml_doc):
        ...
        模板标签遵循模式 {{ ... }} 并用作内容替换的占位符。
        在 XSD 验证之前，它们应从文本内容中移除，同时保留 XML 结构。
